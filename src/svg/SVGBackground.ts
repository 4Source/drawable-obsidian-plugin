...
export class SVGBackground extends SVGSurface {
	...
	constructor(parant: Element) {
		super(parant);
		// Create Dot Pattern
		this.pattern_dot = this.element.createSvg('pattern', { attr: { 'id': 'drawable-dot', 'patternUnits': 'userSpaceOnUse', 'x': 100, 'y': 100, 'width': 20, 'height': 20 } });
		this.pattern_dot.createSvg('circle', { attr: { 'cx': 10.7, 'cy': 10.7, 'r': 0.7, 'fill': 'var(--canvas-dot-pattern)' } });
		// Create Plus Pattern
		this.pattern_plus = this.element.createSvg('pattern', { attr: { 'id': 'drawable-plus', 'patternUnits': 'userSpaceOnUse', 'x': 100, 'y': 100, 'width': 20, 'height': 20 } });
